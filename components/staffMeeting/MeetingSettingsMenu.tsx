...
type ActiveModal = 'info' | 'order' | 'items' | null;
    ...
    <div ref={wrapperRef} className="relative inline-block">
      ...
      {menuOpen && (
        <div className="absolute left-0 top-full z-20 mt-1 flex w-52 flex-col gap-0.5 rounded-lg border border-zinc-200 bg-white p-1 shadow-lg dark:border-zinc-700 dark:bg-zinc-900">
          {canEditInfo && (
            <button type="button" className={MENU_ITEM_BUTTON} onClick={() => openModal('info')}>
              회의정보 관리
            </button>
          )}
      ...
      {activeModal === 'info' && (
        <Modal title="회의정보 관리" onClose={() => setActiveModal(null)}>
          {meetingInfoForm}
        </Modal>
      )}
      {activeModal === 'order' && (
