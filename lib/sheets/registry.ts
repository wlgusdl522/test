...
// 규격/단위/수량/단가/금액/품목명(단일 품목 시절 컬럼)은 기존 행과의 위치 호환을 위해 그대로 두고,
// 여러 품목을 등록할 수 있게 되면서 실제 품목 목록은 맨 뒤에 추가한 품목목록JSON(배열)에 담는다.
// 금액 컬럼은 계속 "총액"(품목목록 합계) 용도로 채워 넣어 기존 코드(메시지/목록 표시)가 그대로 동작하게 한다.
