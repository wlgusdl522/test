...
// 일회성 설정 라우트 — 실행 후 삭제할 것. 후원금/후원물품 상세 명단을 저장할
// '이사회후원상세' 탭이 스프레드시트에 없으면 새로 만든다(이미 있으면 그대로 둔다).
export async function GET() {
  try {
    ...
    const spreadsheetId = BOARD_DONATION_DETAIL_TABLE.spreadsheetId;
    const sheetName = BOARD_DONATION_DETAIL_TABLE.sheetName;
    ...
    const endCol = String.fromCharCode(65 + BOARD_DONATION_DETAIL_TABLE.headers.length - 1);
    await sheets.spreadsheets.values.update({
      spreadsheetId,
      range: `${sheetName}!A2:${endCol}2`,
      valueInputOption: 'RAW',
      requestBody: { values: [BOARD_DONATION_DETAIL_TABLE.headers] },
    });
