...
// 일회성 설정 라우트 — 실행 후 삭제할 것. 전체회의정보 탭이 없으면 새로 만든다.
export async function GET() {
  try {
    ...
    if (실제탭목록.includes(STAFF_MEETING_INFO_TABLE.sheetName)) {
      return NextResponse.json({ result: 'ok', tab: 'exists' });
    }

    await sheets.spreadsheets.batchUpdate({
      spreadsheetId,
      requestBody: { requests: [{ addSheet: { properties: { title: STAFF_MEETING_INFO_TABLE.sheetName } } }] },
    });
