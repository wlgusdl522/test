    ...
    <main className={pageFluid}>
      ...
      <div className="flex items-center gap-2 mb-4">
        ...
        <FormToggle label={editing ? '교통카드 사용 수정' : '교통카드 사용 등록'} defaultOpen={!!editing || isNewParam === '1'}>
          <form action={editing ? updateTransitLedgerAction : addTransitLedgerAction} className="grid grid-cols-2 gap-3">
      ...
      <div className={`${cardTableWrap} mt-3`}><table className={tableClean}>
        <thead>
          <tr>
        ...
        <tbody>
          {records.length === 0 ? (
            <tr><td className={tdClean} colSpan={10}><span className="text-zinc-400">해당 조건에 맞는 내역이 없습니다.</span></td></tr>
          ) : records.map((r) => (
            <tr key={r.id} className={trHoverClean}>
              ...
              <td className={`${tdClean} flex gap-1.5`}>
                <a href={`/transit-card?edit=${r.id}`} className={btnSecondary}>수정</a>
