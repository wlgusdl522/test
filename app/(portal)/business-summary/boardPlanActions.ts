...
export async function saveBoardReportSectionAction(구분: BoardReportType, rows: BoardPlanRowInput[]): Promise<void> {
  await saveBoardReportSection(구분, rows);
...
export async function setReportPeriodAction(formData: FormData): Promise<void> {
  ...
  await setReportPeriod(구분, 기간텍스트);
  revalidatePath('/business-summary/report');
}
